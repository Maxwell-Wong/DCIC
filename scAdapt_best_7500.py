...
def scAdapt(args, data_set, cell_label_digit):
    ## prepare data
    batch_size = args.batch_size
    kwargs = {'num_workers': 0, 'pin_memory': True}

    source_name = args.source_name # "train"
    target_name = args.target_name # "test"
    domain_to_indices = np.where(data_set['accessions'] == source_name)[0]
    print("domain_to_indices_train:",domain_to_indices)
    train_set = {'features': data_set['features'][domain_to_indices], 'labels': data_set['train_labels'],
                 'accessions': data_set['accessions'][domain_to_indices]} # 'labels': data_set['train_labels'][domain_to_indices] data_set['features'][domain_to_indices]


    domain_to_indices = np.where(data_set['accessions'] == target_name)[0]
    test_set = {'features': data_set['features'][domain_to_indices], 'labels': data_set['test_labels'],
                'accessions': data_set['accessions'][domain_to_indices]} # 'labels': data_set['test_labels'][domain_to_indices]


    print('source labels:', np.unique(train_set['labels']), ' target labels:', np.unique(test_set['labels']))

    test_set_eval = {'features': data_set['features'][domain_to_indices], 'labels': data_set['test_labels'],
    ...
    source_loader = torch.utils.data.DataLoader(data, batch_size=batch_size, shuffle=False, drop_last=True, **kwargs)
    
    data = torch.utils.data.TensorDataset(
        torch.FloatTensor(test_set['features']), torch.LongTensor(matrix_one_hot(test_set['labels'], int(max(train_set['labels'])+1)).long()))

    target_loader = torch.utils.data.DataLoader(data, batch_size=batch_size, shuffle=False, drop_last=True, **kwargs)
    ...
    optimizer_centloss = torch.optim.SGD([{'params': center_loss.parameters()}], lr=0.5)   # lr = 0.5
    ...
    parameter_list = base_network.get_parameters() + ad_net.get_parameters() + label_predictor.get_parameters() # 参数列表
    ...
    for epoch in range(args.num_iterations):
        if epoch % (args.epoch_per) == 0 and epoch != 0:
            ...
            predict_prob_arr, predict_label_arr = torch.max(softmax_out, 1)  # torch.max(input, dim) 

            if epoch == args.epoch_th:
                data = torch.utils.data.TensorDataset(torch.FloatTensor(test_set['features']), predict_label_arr.cpu())
                target_loader_align = torch.utils.data.DataLoader(data, batch_size=batch_size, shuffle=False, drop_last=True,
                                                            **kwargs)
            ...
            if args.mode == 1:
                ...
                digit_label_dict = {v:k for k,v in cell_label_digit.items()}

                y_pred_label = [digit_label_dict[x] if x in digit_label_dict else "Unknown" for x in predict_label_arr.cpu().data.numpy()]  
            

                # save  最终输出
                pred_labels_file = result_path + "submission_level" + args.level_num + ".csv"
                pd.DataFrame({'cell_id':data_set['barcode'], 'pred_label':y_pred_label, 'prob': predict_prob_arr.cpu().data.numpy()}).to_csv(pred_labels_file, sep=',', index=False)

                print('iter', epoch, 'finished')
                ...
                print('iter:', epoch, "average acc over all test cell types: ", round(np.nanmean(acc_by_label), 3))   # np.nanmean()计算非空值的算数平均值
                ...
                y_pred = predict_label_arr.cpu().data.numpy()

                micro = f1_score(y_true, y_pred, average='micro')
                macro = f1_score(y_true, y_pred, average='macro')
                weighted = f1_score(y_true, y_pred, average='weighted')

                print('iter', epoch, "F1 micro: ", micro)
                print('iter', epoch, "F1 macro: ", macro)
                print('iter', epoch, "F1 weighted: ", weighted)

                writer.add_scalar('EVAL/F1_micro', micro, epoch)
                writer.flush()
                writer.add_scalar('EVAL/F1_macro', macro, epoch)
                writer.flush()
                writer.add_scalar('EVAL/F1_weighted', weighted, epoch)
                writer.flush()
