...
def scAdapt(args, data_set):
    ...
    for epoch in range(args.num_iterations):
        if epoch % (2500) == 0 and epoch != 0:
            ...
            '''
            print(torch.max(softmax_out, 1) )
            torch.return_types.max(
            values=tensor([0.9998, 0.9996, 0.9987,  ..., 0.3889, 0.7162, 0.3799], device='cuda:0',grad_fn=<MaxBackward0>),
            indices=tensor([1, 1, 1,  ..., 1, 1, 1], device='cuda:0'))
            '''


            # 拒绝域
            for idx, arr in enumerate(predict_prob_arr):  # 若softmax值小于0.5，将其设为Unknown
                if arr < 0.2:
                    predict_label_arr[idx] = -1
            ...
            f = open('tttttme.txt','w')
            f.write('%s' %(str(predict_label_arr)))
            ...
            '''
            .cuda()是读取GPU中的数据, .data是读取Variable中的tensor
            .cpu是把数据转移到cpu上,  .numpy()把tensor变成numpy
            '''

            # else "Unknown"  字典中不存在的值为-1 
            y_pred_label = [digit_label_dict[x] if x in digit_label_dict else "Unknown" for x in predict_label_arr.cpu().data.numpy()]  
            # import pdb; pdb.set_trace()    # if predict_label_arr< 0.5 --> y_pred_label = Unknown
           ...
           # pd.DataFrame([predict_prob_arr.cpu().data.numpy(), y_pred_label],  index=["cell_id", "level1"]).to_csv(pred_labels_file, sep=',')
